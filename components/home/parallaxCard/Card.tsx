...
const Card = ({i,title, description, src, link, color, range, targetScale, progress}:any) => {
        ...
        <div className='flex h-full mt-12 gap-12'>
          ...
          <div className="imgContainer relative w-[60%] h-full rounded-3xl overflow-hidden">
            <motion.div style={{scale:imageScale}} className='w-full h-full'>
              <Image fill src={`https://avatar.iran.liara.run/public`} alt='image' className='object-cover'/>
            </motion.div>
          </div>
        </div>
      </motion.div>
